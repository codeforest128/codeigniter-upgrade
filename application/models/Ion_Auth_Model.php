...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password($password, $salt=false, $use_sha1_override=FALSE)
	{
		...
		if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt')
		{
			...
			print_r($this->bcrypt->hash('kksksy1996'));exit();
		}
    ...
    public function get_admin_users()
    {
        ...
        $this->db->select($this->tables['users'] . '.email');
        $this->db->from($this->tables['users']);
        $this->db->join($this->tables['users_groups'], $this->tables['users_groups'] . '.user_id = ' . $this->tables['users'] . '.id');
        $this->db->join($this->tables['groups'], $this->tables['groups'] . '.id = ' . $this->tables['users_groups'] . '.group_id');
        $this->db->where($this->tables['groups'] . '.name', 'admin');
    ...
    public function get_mailing_list()
    {
        ...
        $query = $this->db->get_where($this->tables['users'], array('mailing_list' => '1'));

        if($query->num_rows() > 0) {
            $mailing_list = $query->result();
        }
